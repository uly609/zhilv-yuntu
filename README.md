...
- `backend/app/services/map_service.py`
  负责对接高德地图 Web 服务，补充地址、经纬度、路线估算和景点图片。
...
- **数据持久化与验证**
  已完成 SQLite 持久化存储，主要 API 测试已补齐，核心链路可以稳定跑通。
...
- **后端架构演进**
  可以继续拆分服务边界，引入 Redis 做缓存、会话态管理、热点目的地数据复用与异步任务解耦。
- **Agent 与编排能力**
  当前以 LangChain 为主，后续可以进一步尝试 LangGraph，把生成、检索、地图 enrich、天气补充、编辑与导出组织成更清晰的状态流。
- **外部工具与 MCP 化**
  地图、天气、POI 检索这类外部能力后续可以逐步抽成 MCP 工具层，便于和不同 Agent 或工作流复用。
- **模型效果提升**
  可以补充 prompt evaluation、输出质量打分、自动回归样例集，并进一步尝试旅行场景的指令微调或偏好对齐。
- **质量评估体系**
  后续可以建立 itinerary 质量指标，例如结构完整性、预算合理性、地图命中率、天气一致性和用户指令满足度。
- **性能与稳定性**
